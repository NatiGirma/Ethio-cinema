    ...
    movies: [
      {
        title: "Wend Barya",
        trailer: "https://www.youtube.com/embed/dQw4w9WgXcQ",
        image: "wend_barya.jpg",
      },
      {
        title: "Enchewawot",
        trailer: "https://www.youtube.com/embed/dQw4w9WgXcQ",
        image: "enchewawot.jpg",
      },
    ],
  },
  {
...
document.getElementById("search-city").addEventListener("click", () => {
  ...
  const selectedCinema = cinemas.find((cinema) =>
    cinema.address.includes(selectedLocation)
  );

  if (selectedCinema) {
    map.setView([selectedCinema.latitude, selectedCinema.longitude], 13);
    L.marker([selectedCinema.latitude, selectedCinema.longitude])
      .addTo(map)
      .bindPopup(selectedCinema.name)
      .openPopup();
    updateResults(
      selectedCinema.name,
...
function findNearestCinema(latitude, longitude) {
  ...
  // Find the nearest cinema
  const nearestCinema = distances.reduce((a, b) =>
  ...
  updateResults(
    nearestCinema.name,
    nearestCinema.address,
    nearestCinema.movies
  );
}
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    alert(
      `Thank you, ${clientName}! You have purchased a ticket for "${movieTitle}" using ${paymentMethod}. Your chair number is ${chairNumber}.`
    );
...
document
  .getElementById("register-cinema")
  .addEventListener("click", function () {
    const cinemaName = document.getElementById("cinema-name-input").value;
    const cinemaLocation = document.getElementById(
    ...
    if (cinemaName && cinemaLocation && movieTitle && showtime && fee) {
      ...
      ).textContent = `Thank you for joining us, ${cinemaName}! You have successfully registered your cinema.`;
